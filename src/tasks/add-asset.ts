...
task(TASK_ADD_ASSET)
    ...
    .setAction(async function (taskArgs: AddAssetArgs, hre) {
        ...
        if (
            ...
            !config.krAssetConfig &&
            !config.scdpDepositConfig &&
            !config.scdpKrAssetConfig
        ) {
        ...
        if (found === -1) {
            ...
            hre.krAssets = hre.krAssets.map(c =>
                c.address === asset.address ? (asset as TestAsset<KreskoAsset, any>) : c,
            );
