...
export const slots = {
    ...
    kreskoAssetDebt: 4,
    mintedKreskoAssets: 5,
    maxLiquidationRatio: 11,
    feeRecipient: 12,
    minCollateralRatio: 13,
    minDebtValue: 14,
...
export async function getAccountMintedAssets(address: string) {
    try {
        const [dataSlot, length] = await getMappingArray(
            increaseHexBy(slots.minter, slots.mintedKreskoAssets),
            address,
        );
        if (!length) return [];
...
export async function getIsRebased<T extends KreskoAsset | ERC20Upgradeable>(asset: MockContract<T>) {
    ...
    try {
        ...
        try {
            denominator = BigNumber.from(
                hexStripZeros(await getStorageAt(asset.address, slots.kreskoAssetIsRebased + 1)),
            );
        } catch {}
