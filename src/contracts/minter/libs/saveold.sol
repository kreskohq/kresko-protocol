...
library LibCalculation {
    struct MaxLiquidationVars {
        FixedPoint.Unsigned maxLiquidatableUSD;
        FixedPoint.Unsigned liquidationIncentive;
        FixedPoint.Unsigned seizeCollateralRate;
        FixedPoint.Unsigned seizeCollateralValue;
        FixedPoint.Unsigned repayDebtRate;
        FixedPoint.Unsigned repayDebtValue;
    }
    ...
    function getMaxLiquidation(
        ...
        CollateralAsset memory collateral = self.collateralAssets[_seizedCollateral];
        FixedPoint.Unsigned memory debtFactor = _repayKreskoAsset.kFactor.mul(self.liquidationThreshold).div(
            collateral.factor
        );
        maxLiquidatableUSD = minCollateralRequired
            .sub(accountCollateralValue)
            .div(calcValueGainedPerUSDRepaid(debtFactor, _repayKreskoAsset, collateral))
            .div(debtFactor)
            .div(collateral.factor);
        ...
        return maxLiquidatableUSD.mul(FixedPoint.Unsigned(1 ether + Constants.BASIS_POINT));
    }
