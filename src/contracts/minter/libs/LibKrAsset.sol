...
library LibKrAsset {
     ...
     * @return The value for the provided amount of the Kresko asset.
     */
    function getKrAssetValueRedstone(
        MinterState storage self,
        address _kreskoAsset,
        uint256 _amount,
        bool _ignoreKFactor
        ...
        KrAsset memory krAsset = self.kreskoAssets[_kreskoAsset];
        uint256 value = LibPrice.getPrice(krAsset.redstone).wadMul(_amount);
     ...
     * @param _ratio The collateralization ratio required: higher ratio = more collateral required.
     */
    function getMinimumCollateralValueAtRatioRedstone(
        MinterState storage self,
        address _krAsset,
        uint256 _amount,
        uint256 _ratio
        ...
        // Calculate the collateral value required to back this Kresko asset amount at the given ratio
        return self.getKrAssetValueRedstone(_krAsset, _amount, false).wadMul(_ratio);
    }
}
