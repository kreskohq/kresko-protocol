...
library DSCore {
    ...
    /**
     * @dev Emitted when the diamond initialization functionality is used.
     * @param version Storage version of the contract.
     * @param sender msg.sender to initialize.
     * @param initializer Address of the initializer contract.
     * @param diamondOwner Diamond owner at the time.
     * @param totalFacets Total facet contract count.
     * @param timestamp Timestamp for this initialization.
     */
    ...
    function create(FacetCut[] memory _initialFacets, Initializer[] memory _initializers, address _contractOwner) internal {
        ...
        if (self.initialized) revert DIAMOND_ALREADY_INITIALIZED();
        ...
        cut(_initialFacets, address(0), ""); // just the facets, run all initializers after

        Auth._grantRole(Role.ADMIN, _contractOwner);

        for (uint256 i; i < _initializers.length; i++) {
            execute(_initializers[i].initContract, _initializers[i].initData);
        }
    }

    /// @notice Executes @param _initializer with @param _calldata using delegatecall.
    function execute(address _initializer, bytes memory _calldata) internal {
    ...
    function cut(FacetCut[] memory _diamondCut, address _initializer, bytes memory _calldata) internal {
        ...
        execute(_initializer, _calldata);
    }
