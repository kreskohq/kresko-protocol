...
describe("Kresko", function () {
    ...
    describe("Kresko Assets", function () {
        ...
        describe("#addKreskoAsset", function () {
            ...
            it("should not allow non-owner to add assets", async function () {
                await expect(
                    this.Kresko.connect(this.signers.userOne).addKreskoAsset(ADDRESS_ONE, SYMBOL_TWO, ONE, ADDRESS_TWO),
                ).to.be.revertedWith("Ownable: caller is not the owner");
            });
        });
    ...
    describe("Global variables", function () {
        ...
        describe("#updateCloseFactor", function () {
            ...
            it("should emit CloseFactorUpdated event", async function () {
                ...
                const { args } = await extractEventFromTxReceipt<CloseFactorUpdatedEvent>(
                    receipt,
                    "CloseFactorUpdated",
                );
                expect(args.closeFactor).to.equal(validCloseFactor);
            });
            ...
            it("should not allow the close factor to be updated by non-owner", async function () {
                await expect(
                    this.Kresko.connect(this.signers.userOne).updateCloseFactor(validCloseFactor),
                ).to.be.revertedWith("Ownable: caller is not the owner");
            });
        });
    ...
    describe("Liquidations", function () {
        ...
        describe("#liquidate", function () {
            ...
            it("should emit LiquidationOccurred event", async function () {
                ...
                const maxPossibleSeizedAmount = Number(beforeUserOneDebtAmount.mul(closeFactor));
                expect(Number(fromFixedPoint(args.collateralSent))).to.be.lessThanOrEqual(maxPossibleSeizedAmount);
            });
            ...
            it("should send the liquidator whole collateral and keep debt position when _keepKrAssetDebt = true", async function () {
                ...
                expect(liquidatorCollateralBalanceInWalletAfterLiquidation).to.be.equal(fromBig(args.collateralSent));
