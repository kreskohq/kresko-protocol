...
describe("KreskoAsset", function () {
    ...
    beforeEach(async function () {
        ...
        this.kreskoAsset = <KreskoAsset>await (
            await hre.upgrades.deployProxy(
                kreskoAssetFactory,
                [name, symbol, this.signers.admin.address, this.signers.operator.address],
                {
    ...
    describe("#initialize", function () {
        it("should initialize the contract with the correct parameters", async function () {
            ...
            expect(
                await this.kreskoAsset.hasRole(this.kreskoAsset.OPERATOR_ROLE(), this.signers.operator.address),
            ).to.equal(true);
        });

        it("should not allow being called more than once", async function () {
            expect(
                this.kreskoAsset.initialize("foo", "bar", this.signers.admin.address, this.signers.operator.address),
    ...
    describe("#mint", function () {
        ...
        it("should allow the operator to mint to any address", async function () {
            ...
            expect(await this.kreskoAsset.balanceOf(this.signers.operator!.address)).to.equal(0);

            await this.kreskoAsset.connect(this.signers.operator).mint(this.signers.operator!.address, this.mintAmount);

            // Check total supply and user's balances increased
            expect(await this.kreskoAsset.totalSupply()).to.equal(this.mintAmount);
            expect(await this.kreskoAsset.balanceOf(this.signers.operator!.address)).to.equal(this.mintAmount);
        });
        ...
        it("should allow the operator to mint to owner's address", async function () {
            ...
            await this.kreskoAsset.connect(this.signers.operator).mint(this.signers.admin.address, this.mintAmount);
        ...
        it("should not allow non-operator addresses to mint tokens", async function () {
            ...
            await expect(
                this.kreskoAsset.connect(this.signers.admin).mint(this.signers.operator!.address, this.mintAmount),
            ).to.be.revertedWith(
                `AccessControl: account ${this.signers.admin.address.toLowerCase()} is missing role 0x8952ae23cc3fea91b9dba0cefa16d18a26ca2bf124b54f42b5d04bce3aacecd2`,
            );
    ...
    describe("#burn", function () {
        ...
        it("should allow the operator to burn tokens from user's address (without token allowance)", async function () {
            ...
            await this.kreskoAsset.connect(this.signers.operator).burn(this.signers.admin.address, this.mintAmount);
        ...
        it("should allow the operator to burn tokens from user's address without changing existing allowances", async function () {
            await this.kreskoAsset.connect(this.signers.admin).approve(this.signers.operator.address, this.mintAmount);

            expect(await this.kreskoAsset.totalSupply()).to.equal(this.mintAmount);
            expect(
                await this.kreskoAsset.allowance(this.signers.admin.address, this.signers.operator!.address),
        ...
        it("should not allow non-operator addresses to burn tokens", async function () {
            await expect(
                this.kreskoAsset.connect(this.signers.userTwo).burn(this.signers.admin.address, this.mintAmount),
            ).to.be.revertedWith(
                `AccessControl: account ${this.signers.userTwo.address.toLowerCase()} is missing role 0x8952ae23cc3fea91b9dba0cefa16d18a26ca2bf124b54f42b5d04bce3aacecd2`,
            );
