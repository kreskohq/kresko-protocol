...
import { Kresko } from "../../typechain";
import { Signers } from "../../types";

import { addNewKreskoAssetWithOraclePrice, deployAndWhitelistCollateralAsset } from "../Kresko";
...
describe.skip(`loops and gas consumption - gasLimit: ${gasLimit}`, function () {
    ...
    it("whitelistCollateral-depositCollateral-partialCollateralWithdraw 500 runs", async function () {
        ...
        const userOneKresko: Kresko = this.kresko.connect(this.userOne);
        let targetRound = 500;
        let round = 0;
        let lastTxCost = 0;
        for (round; round < targetRound; round++) {
            ...
            await collateralAsset.setBalanceOf(this.userOne.address, depositValue);
    ...
    it("whitelistCollateral-depositCollateral-partialCollateralWithdraw two users 500 runs", async function () {
        ...
        const withdrawValue = parseEther("2");
        const userOneKresko: Kresko = this.kresko.connect(this.userOne);
        const userTwoKresko: Kresko = this.kresko.connect(this.userTwo);
        ...
        for (round; round < targetRound; round++) {
            ...
            await collateralAsset.setBalanceOf(this.userOne.address, depositValue);
            await collateralAsset.setBalanceOf(this.userTwo.address, depositValue);
            await userOneKresko.depositCollateral(collateralAsset.address, depositValue);
        ...
        const collateralDeposits = await (this.kresko as Kresko).getDepositedCollateralAssets(this.userOne.address);
    ...
    it("should not increase gas cost (delta 10 wei) when withdrawing whole collateral over 100 runs", async function () {
        ...
        for (round; round < targetRound; round++) {
            ...
            const tx = await collateralAsset.setBalanceOf(this.userOne.address, depositValue);
            await tx.wait();
            const depositTx = await userOneKresko.depositCollateral(collateralAsset.address, depositValue);
